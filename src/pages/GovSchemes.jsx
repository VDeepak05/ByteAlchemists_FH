...
const GovSchemes = () => {
    ...
    const categories = ['All Schemes', 'Income Support', 'Insurance', 'Organic Farming', 'Livestock', 'Irrigation'];
    ...
    const categoryColors = {
        ...
        'State Initiative': 'bg-[#16a34a]/90',
        'Infrastructure': 'bg-[#0891b2]/90',
        'Livestock': 'bg-[#7c3aed]/90',
    };

    const getEligibilityBadge = (scheme) => {
        // Simple eligibility logic based on category
        if (scheme.category === 'Income Support' || scheme.category === 'State Initiative') {
            return { text: 'Likely Eligible', color: 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400', icon: 'check_circle' };
        } else if (scheme.category === 'Infrastructure') {
            return { text: 'Ineligible', color: 'bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400', icon: 'cancel' };
        }
    ...
    const filteredSchemes = schemesData.filter(scheme => {
        ...
        const matchesCategory = selectedCategory === 'All Schemes' || selectedCategory === 'All' || scheme.category === selectedCategory;
        ...
        <div className="max-w-[1200px] mx-auto space-y-8">
            ...
            <div className="flex flex-col md:flex-row justify-between items-end gap-6">
                <div className="flex flex-col gap-2 max-w-2xl">
                    <h1 className="text-slate-900 dark:text-white text-4xl font-black leading-tight tracking-tight">Government Agricultural Schemes</h1>
            ...
            <div className="flex flex-col gap-4">
                <div className="bg-white dark:bg-[#1a2e22] p-2 rounded-xl shadow-sm border border-[#e8f3ec] dark:border-[#1e3a29] flex flex-col md:flex-row gap-2">
                    <div className="flex-1 flex items-center px-4 gap-3 border-b md:border-b-0 md:border-r border-[#e8f3ec] dark:border-[#1e3a29] py-2 md:py-0">
                        ...
                        <input
                            className="w-full bg-transparent border-none focus:ring-0 focus:outline-none text-slate-900 dark:text-white placeholder:text-[#50956a]"
                    ...
                    <div className="flex items-center gap-2 px-4 py-2 md:py-0">
                        ...
                        <select
                            className="bg-transparent border-none focus:ring-0 text-sm font-medium text-slate-900 dark:text-white"
                    ...
                    <button className="bg-primary text-white px-8 py-3 rounded-lg font-bold hover:shadow-lg hover:shadow-primary/20 transition-all">
                        Search
                    </button>
                </div>
                    ...
                    {categories.map((cat, idx) => (
                        <button
                            ...
                            className={`flex h-9 shrink-0 items-center justify-center gap-x-2 rounded-full px-5 transition-colors ${selectedCategory === cat
                                    ...
                                    : 'bg-[#e8f3ec] dark:bg-[#1e3a29] hover:bg-primary/20 text-slate-900 dark:text-white'
                                }`}
                        >
                            ...
                            <div className="p-6 flex flex-col flex-1 gap-4">
                                <div>
                                    <div className="flex justify-between items-start mb-2">
                                        <h3 className="text-xl font-bold text-slate-900 dark:text-white">{scheme.name}</h3>
